...
# Usage: bash scripts/set-gemini-key.sh AIzaSy...
...
if [[ ! "$KEY" =~ ^AIza[0-9A-Za-z_-]{20,}$ ]]; then
  echo "ERROR: key should look like AIzaSy... (from Google AI Studio, not a project ID)."
  exit 1
fi
...
set_or_append GEMINI_MODEL "gemini-2.0-flash"
